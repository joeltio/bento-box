...
struct TestComponent : public ics::component::UserComponent {
    ...
    TestComponent(int width, int height) : UserComponent(TEST_COMPONENT_NAME, createCompDef()) {
...
class StoresFixture : public::testing::Test {
   protected:
    ...
    // First entity
    ics::index::EntityIndex::EntityId entity1Id = indexStore.entity.addEntityId();
    TestComponent comp1{50, 30};

    // Second entity
    ics::index::EntityIndex::EntityId entity2Id = indexStore.entity.addEntityId();
    TestComponent comp2{90, 10};

    static bento::protos::AttributeRef createAttrRef(ics::index::EntityIndex::EntityId entityId, const char* attrName) {
...
TEST_F(StoresFixture, GetAndSetAttributeRef) {
    ...
    ASSERT_EQ(cppAttrRef.primitive().int_64(), comp1.getValue("height").primitive().int_64());
...
TEST_F(StoresFixture, RetrieveNode) {
    ...
    auto retrieveAttr = retrieveNode.mutable_retrieve_op()->mutable_retrieve_attr();
    ...
    ASSERT_EQ(value.primitive().int_64(), comp1.getValue("width").primitive().int_64());
}
